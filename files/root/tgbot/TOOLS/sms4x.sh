...
if [ -f "/root/smsid_4x" ]; then
    sms_id=$(head -n 1 /root/smsid_4x)
else
    echo "Berkas smsid not found"
    
fi

cek_id=$(adb -s a57668797d04 shell content query --uri content://sms/inbox --sort "date DESC" | grep 'Row: 0' | awk '{print $3}')
...
echo "sms update"
echo $cek_id > /root/smsid_4x

sms_raw=$(adb -s a57668797d04 shell content query --uri content://sms/inbox --sort "date DESC" | tr '\r\n' ' ' | sed 's/Row: /\nSMS_/g')
